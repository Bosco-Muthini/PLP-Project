...
def add_forest(request):
    ...
    if request.method == 'POST':
        ...
        if not description:
            messages.error(request, 'The name of farmer or your group is required!')
            return render(request, 'forests/add_forest.html', context)
...
def export_csv(request):
    ...
    response['Content-Disposition']= 'attachment; filename=Trees Record '+ str(datetime.datetime.now())+'.csv'
...
def export_pdf(request):
    ...
    response['Content-Disposition'] = 'attachment; filename="Trees Periodic Updates.pdf"'
...
image_path = os.path.join('reforestation','static', 'img', 'poster_2024-01-30-025944.png')
